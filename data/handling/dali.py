...
# TODO: cleanup/rename (actually probably everything should be cleaned up)
# a DALI pipeline closure for creating vector input-image output pairs
# the vector is generated by random_vector_func
def dali_random_vector_to_image_pipeline(data_dir: str, metadata_filename: str, vector_func):
    input = dali.ops.FileReader(file_root=data_dir, file_list=metadata_filename, random_shuffle=True)
    decode = dali.ops.ImageDecoder()
    cast = dali.ops.Cast(dtype=dali.types.DALIDataType.FLOAT)  # TODO: better data type?
    transpose = dali.ops.Transpose(perm=[2, 0, 1], device='gpu')
    vector = TorchPythonFunction(vector_func)

    def _apply():
        img, _ = input()  # ignore labels
        img = decode(img)
        img = cast(img)
        img = transpose(img.gpu())
        # TODO: fix
...
# TODO: cleanup
def dali_image_to_vector_pipeline(data_dir: str, metadata_filename: str, vector_func):
    input = dali.ops.FileReader(file_root=data_dir, file_list=metadata_filename, random_shuffle=True)
    decode = dali.ops.ImageDecoder()
    cast = dali.ops.Cast(dtype=dali.types.DALIDataType.FLOAT)  # TODO: better data type?
    transpose = dali.ops.Transpose(perm=[2, 0, 1], device='gpu')
